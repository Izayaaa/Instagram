...
def get_link_image():
    ...
    if resp.status_code == 200:
        ...
        html_link = resp.text
        find_link = re.search(r'<meta property="og:image"\ content="(.*?)" />',
     html_link)
    else:
        print('Link error, try another link.')
    if find_link:
        image_link = find_link.group(1)
    fileName = datetime.datetime.now().strftime('%Y-%m-%d_') +\
     str(randint(0, 10000)) + '.jpg'
    return image_link, fileName


def get_link_video():
    input_data = input('Instagram Image Link: ')
    resp = requests.get(input_data, stream=True)
    print('Checking link ...')
    ...
    if resp.status_code == 200:
        ...
        html_link = resp.text
        find_link = re.search(r'<meta property="og:video" content="(.*?)" />',
     html_link)
...
def run_script():
    ...
    if number == str(1):
        ...
        print('Input number, please.'); msvcrt.getch();run_script()


run_script()
print('\nPress any key to exit...'); msvcrt.getch()
